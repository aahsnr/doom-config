...
** Jupyter
#+begin_src emacs-lisp
(defun jupyter-org-src-block-params--advice (old-fn &rest args)
  (let ((result (apply old-fn args)))
    (when (and result (null (nth 3 result)))
      ;; If `org-babel-src-block-end' fails and returns nil, fall back to the
      ;; start of the block (nth 2 of result) to prevent a crash.
      (let ((end-pos (or (org-babel-src-block-end) (nth 2 result))))
        (when end-pos
          ;; Use `setcar` on `nthcdr` to robustly modify the list element.
          (let ((tail (nthcdr 3 result)))
            (when tail
              (setcar tail (copy-marker end-pos)))))))
    result))

(after! org
  ;; Force the loading of the ob-jupyter library. This is a more direct
  ;; approach to ensure that its functions, like `org-babel-execute:jupyter-python`,
  ;; are defined and available to Org Babel.
  (require 'ob-jupyter)

  ;; After ensuring the library is loaded, we still register 'jupyter' with Org Babel.
  ;; This makes the solution more robust.
  (org-babel-do-load-languages
   'org-babel-load-languages
   '((jupyter . t)))
  (advice-add 'jupyter-org-src-block-params :around #'jupyter-org-src-block-params--advice)
...
* Jupyter
#+begin_src emacs-lisp
;; --- Fix for jupyter-org and corfu error ---
#+end_src
* Docker
#+begin_src emacs-lisp
