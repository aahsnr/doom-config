...
(after! jupyter
  ;; Set the Python interpreter for the server.
  (setq-hook! 'jupyter-python-mode-hook
  ...
  ;; Doom's org module often handles many mime types. If you encounter issues,
...
;; This is important for Doom Emacs's lazy loading to ensure kernels are recognized.
(with-eval-after-load 'ob-jupyter
  (org-babel-jupyter-aliases-from-kernelspecs))

;; --- LSP Integration (assuming :lang python and +lsp flag are enabled) ---
(after! lsp-mode
  ;; Example: Set LSP priority for pyright (adjust as needed if you use other LSP servers)
  (set-lsp-priority! 'pyright 2))

  ;; If you use `direnv` module, LSP might automatically pick up the virtual environment.
  ;; Otherwise, you might need:
  ;; (add-hook 'jupyter-python-mode-hook #'lsp-pyright-activate-venv)


;; --- Completion and Snippets (assuming :completion corfu is enabled) ---
(after! corfu
  ;; Enable Corfu for relevant modes, including Jupyter Python source blocks.
  (setq corfu-auto-completion-modes '(python-mode
                                      org-mode
                                      org-src-mode
                                      jupyter-python-mode))

  ;; Configure Corfu backend and sorting.
  (setq corfu-backend '(cape-dabbrev cape-file cape-keyword nil)
        corfu-sort-function #'corfu-sort-by-length
        corfu-doc-indicator nil) ; Set to `t` to show documentation in a separate buffer

  ;; Add Corfu to org-src-mode hook for completion within source blocks.
  (add-hook 'org-src-mode-hook #'corfu-mode)

  ;; Exclude specific modes from Corfu if their completion is already handled
  ;; by another completion system (e.g., `vertico-mode` for minibuffer).
  ;;(add-to-list 'corfu-excluded-modes '(vertico-mode))

  ;; Fix `RET` (Enter) key behavior in Org mode when Corfu is active to ensure
  ;; it inserts the selected completion candidate instead of a newline.
  ...
  ;; Doom Emacs typically handles snippets via its `snippets` module or `yasnippet`
  ;; integration, so explicit `yas-minor-mode` configuration might not be needed.


;; --- Code Formatting (assuming :tools format and :lang python are enabled with black) ---
(use-package! python-black
  :after org ; Ensure `org` module is loaded before configuring `python-black`
  :config
  ;; Enable `black` formatting on save for `python-mode` buffers.
  (add-hook 'python-mode-hook #'black-on-save-mode)
  ;; Enable `black` formatting on save specifically for `jupyter-python` source blocks in Org mode.
  (add-hook 'org-src-mode-hook (lambda ()
                                  (when (string-equal (org-src-get-lang) "jupyter-python")
                                    (add-hook 'after-save-hook #'black-on-save-mode nil 'local)))))

;; Custom keybinding for manual formatting of the current Org source block.
;; This is optional, as format-on-save is often preferred.
...
(defun +my/jupyter-format-src-block ()
  "Format the current Org source block using black if it's a jupyter-python block."
  (interactive)
  (when (string-equal (org-src-get-lang) "jupyter-python")
    (black-buffer)))

;; --- Interactive Debugging with DAP (assuming :tools debugger and +lsp are enabled) ---
(after! dap-python
  ;; Recommend using `debugpy` as the Python debugger with DAP.
  (setq dap-python-debugger 'debugpy)
  ...
  (dap-register-debug-template
   ...
   (list :type "python"
         ...
         ;; This line retrieves connection information from the jupyter package.
...
(map! :map org-mode-map
      ...
      "b" #'dap-breakpoint-toggle)            ;; (b)reakpoint toggle
#+end_src
* Docker
#+begin_src emacs-lisp
