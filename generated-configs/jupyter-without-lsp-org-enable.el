...
;; Uses `lsp-babel` to activate the `pyright` language server inside Org
;; source blocks, providing definitions, references, and diagnostics.
(use-package! lsp-babel
  :after (lsp org)
  :hook (org-mode . #'lsp-babel-enable)
  :config
  ;; This gives you control over when LSP activates inside the C-c ' editor.
  (setq lsp-babel-manual-activation-in-edit-buffers t))
